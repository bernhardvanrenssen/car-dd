...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // ---------------------------
    // EXPORT FUNCTIONALITY - Refined export
    // ---------------------------

    // Camera State Export Solution - No backticks, split script tags
    const createCameraStateExport = () => {
        if (!confirm('Export viewer with current camera positions?')) {
            return;
        }
        
        // Create status popup
        const statusPopup = document.createElement('div');
        statusPopup.id = 'exportPopup';
        ...
        statusPopup.innerHTML = '<h3 style="margin-top: 0; color: #333;">Creating Viewer Export</h3>' +
            '<p id="exportStatus">Capturing camera positions...</p>' +
            '<div style="background: #f1f1f1; height: 20px; border-radius: 10px; margin: 15px 0;">' +
            '<div id="exportProgressBar" style="background: #28a745; height: 100%; width: 10%; border-radius: 10px; transition: width 0.3s;"></div>' +
            '</div>';
        ...
        setTimeout(() => {
            try {
                // 1. Capture current camera positions
                updateProgress(20, 'Capturing camera positions...');
                const cameraData = [];
                
                if (window.meshObjects && window.meshObjects.cameraMarkers) {
                    window.meshObjects.cameraMarkers.forEach(marker => {
                        // Skip if no camera data
                        if (!marker.cameraData) return;
                        
                        // Get current position
                        const pos = marker.getPosition();
                        
                        // Clone the camera data
                        const cameraInfo = {
                            imageId: marker.cameraData.imageId,
                            position: { x: pos.x, y: pos.y, z: pos.z }, // Use CURRENT position
                            quaternion: marker.cameraData.quaternion,
                            cameraId: marker.cameraData.cameraId,
                            imageName: marker.cameraData.imageName,
                            cluster: marker.cameraData.cluster
                        };
                        
                        cameraData.push(cameraInfo);
                    });
                }
                
                console.log('Captured ' + cameraData.length + ' camera positions');
                
                if (cameraData.length === 0) {
                    throw new Error('No camera positions found! Please load camera data first.');
                }
                
                // 2. Get the current HTML content
                updateProgress(40, 'Capturing HTML content...');
                const originalHtml = document.documentElement.outerHTML;
                
                // 3. Create a serialized camera data string
                updateProgress(60, 'Serializing camera data...');
                const serializedCameras = JSON.stringify(cameraData);
                
                // 4. Create viewer code with embedded camera data
                updateProgress(80, 'Creating export code...');
                
                // Get camera initialization code from your existing code - WITHOUT BACKTICKS
                const cameraInitCode = 
    '// Initialize camera markers with preloaded data\n' +
    'function initializeEmbeddedCameras() {\n' +
    '    console.log("Initializing embedded cameras...");\n' +
    '    try {\n' +
    '        // Parse the embedded camera data\n' +
    '        const embeddedCameraData = ' + serializedCameras + ';\n' +
    '        \n' +
    '        if (!embeddedCameraData || !Array.isArray(embeddedCameraData) || embeddedCameraData.length === 0) {\n' +
    '            console.error("No embedded camera data found!");\n' +
    '            return;\n' +
    '        }\n' +
    '        \n' +
    '        console.log("Initializing " + embeddedCameraData.length + " embedded cameras");\n' +
    '        \n' +
    '        // Create camera markers from the embedded data\n' +
    '        createCameraMarkers(app, embeddedCameraData);\n' +
    '        \n' +
    '        // Enable camera detection by default\n' +
    '        const cameraDetectButton = Array.from(document.querySelectorAll("button")).find(\n' +
    '            function(button) { return button.textContent === "Enable Camera Detection"; }\n' +
    '        );\n' +
    '        \n' +
    '        if (cameraDetectButton) {\n' +
    '            console.log("Enabling camera detection...");\n' +
    '            cameraDetectButton.click();\n' +
    '        }\n' +
    '        \n' +
    '        // Hide the camera markers but keep them functional\n' +
    '        if (window.meshObjects && window.meshObjects.cameraMarkers) {\n' +
    '            window.meshObjects.cameraMarkers.forEach(function(marker) {\n' +
    '                if (marker && marker.enabled !== undefined) {\n' +
    ...
    '        \n' +
    '        // Hide debug objects\n' +
    '        if (window.meshObjects && window.meshObjects.debugObjects) {\n' +
    '            window.meshObjects.debugObjects.forEach(function(obj) {\n' +
    '                if (obj && obj.enabled !== undefined) {\n' +
    '                    obj.enabled = false;\n' +
    '                }\n' +
    '            });\n' +
    ...
    '        }\n' +
    '        \n' +
    '        console.log("Embedded cameras initialized successfully");\n' +
    '        return true;\n' +
    '    } catch (err) {\n' +
    '        console.error("Error initializing embedded cameras:", err);\n' +
    '        return false;\n' +
    '    }\n' +
    '}\n' +
    '\n' +
    '// Hide development buttons\n' +
    ...
    '        if (buttonsToHide.includes(button.textContent.trim())) {\n' +
    ...
    '    \n' +
    '    // Hide development buttons immediately\n' +
    '    hideDevButtons();\n' +
    '    \n' +
    ...
    '            setTimeout(function() {\n' +
    '                initializeEmbeddedCameras();\n' +
    '            }, 1000);\n' +
    '        }\n' +
    '    }, 500);\n' +
    '});';
                
                // 5. Create the HTML with embedded camera data
                updateProgress(90, 'Generating export HTML...');
                
                // Insert the initialization code before the closing body tag - WITH SPLIT SCRIPT TAG
                let exportHtml = originalHtml.replace('</body>', '<script>' + cameraInitCode + '</scr' + 'ipt></body>');
                ...
                link.href = url;
                link.download = 'supersplat-viewer-with-cameras.html';
                
                updateProgress(100, 'Export complete!');
                
                // Show the download button
                setTimeout(() => {
                    statusPopup.innerHTML = '<h3 style="margin-top: 0; color: #28a745;">Export Complete!</h3>' +
                        '<p>Successfully captured ' + cameraData.length + ' camera positions.</p>' +
                        '<div style="border-left: 4px solid #f60; padding-left: 15px; margin: 15px 0; background: #fff9f5; padding: 10px;">' +
                        '<h4 style="margin-top: 0; color: #d63031;">Deployment Instructions</h4>' +
                        '<ol>' +
                        '<li>Download the HTML file using the button below</li>' +
                        '<li>Copy the HTML file and the <code>images/</code> folder to your server</li>' +
                        '<li>When opened, the viewer will automatically:</li>' +
                        '<ul>' +
                        '<li>Initialize cameras in their current positions</li>' +
                        '<li>Hide development buttons</li>' +
                        '<li>Enable camera detection</li>' +
                        '</ul>' +
                        '</ol>' +
                        '</div>' +
                        '<div style="display: flex; justify-content: space-between; margin-top: 20px;">' +
                        '<button id="downloadBtn" style="background: #28a745; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer;">Download Viewer</button>' +
                        '<button id="closeBtn" style="background: #6c757d; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer;">Close</button>' +
                        '</div>';
                ...
                statusPopup.innerHTML = '<h3 style="margin-top: 0; color: #dc3545;">Export Failed</h3>' +
                    '<p>' + err.message + '</p>' +
                    '<button id="closeErrBtn" style="background: #6c757d; color: white; padding: 8px 16px; border: none; border-radius: 4px; cursor: pointer; margin-top: 15px;">Close</button>';
                
                document.getElementById('closeErrBtn').addEventListener('click', () => {
                    document.body.removeChild(statusPopup);
                });
            }
        }, 100);
    ...
    (function initExportBtn() {
        ...
        btn.addEventListener('click', createCameraStateExport);
