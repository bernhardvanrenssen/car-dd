...
const serializeViewer = async (splats: Splat[], options: ViewerExportSettings, writer: Writer) => {
    // If both versions are requested, handle each separately
    if (options.versionType === 'both') {
        // Create both dev and client versions
        
        // For the dev version - use your dev template
        const devOptions: ViewerExportSettings = {
            ...options,
            versionType: 'dev' as const,
            filename: options.filename?.replace('.html', '-dev.html')
        };
        
        // Create a new writer for the client version
        const clientWriter = new BufferWriter();
        const clientOptions: ViewerExportSettings = {
            ...options,
            versionType: 'client' as const,
            filename: options.filename?.replace('.html', '-client.html')
        };
        
        // Process dev version with original writer
        await processViewerVersion(splats, devOptions, writer);
        
        // Process client version with new writer
        await processViewerVersion(splats, clientOptions, clientWriter);
        
        // Download the client version
        if (options.type === 'html') {
            const clientBuffer = clientWriter.close();
            download(clientOptions.filename, clientBuffer);
        }
    } else {
        // Just process the requested version (or default to dev if not specified)
        const versionType = (options.versionType || 'dev') as 'dev' | 'client';
        await processViewerVersion(splats, { 
            ...options, 
            versionType 
        }, writer);
    }
...
// Helper function to process a single version
const processViewerVersion = async (splats: Splat[], options: ViewerExportSettings, writer: Writer) => {
    const { experienceSettings, versionType } = options;
    ...
    if (options.type === 'html') {
        ...
        // Load the appropriate template based on version type
        let template = indexHtml;
        let cssTemplate = indexCss;
        let jsTemplate;
        
        // Select the correct JS template based on version type
        if (versionType === 'dev') {
            jsTemplate = indexDevJs;
        } else {
            jsTemplate = indexClientJs;
        }
        ...
        // Select the appropriate JS template based on version type
        let jsTemplate;
        if (versionType === 'dev') {
            jsTemplate = indexDevJs;
        } else {
        ...
        await zipWriter.file('index.js', jsTemplate);
